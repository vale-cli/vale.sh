...
brand — Microsoft and AWS, for example — so `github` falls back to your organization's
...
adopter's `url`, don't add it here too — the linter rejects it.
...
whole section once nothing is left — as soon as it's over. The linter prints
...
The documentation lives at [docs.vale.sh][docs], published from GitBook and
synced to the `docs/` directory of this repository. The two are kept in step
automatically: a commit to `docs/` on this branch updates the site, and an edit
made in GitBook lands here as a commit.

So you can contribute either way. Editing the Markdown under `docs/` and
opening a pull request is the usual route — your PR gets a preview URL as a
status check, so you can see the rendered page before it merges.

Two things not to do: `docs/SUMMARY.md` is the table of contents and is
generated by GitBook, so edit the page tree in GitBook rather than by hand; and
`.gitbook.yaml` tells the sync that content lives in `docs/`, so leave it where
it is.

The site used to render these pages itself from `src/lib/content`. It no
longer does — `vale.sh/docs/*` redirects to `docs.vale.sh` (see
`netlify.toml`).
...
Docker Hub, PyPI, conda-forge, Homebrew, Chocolatey, and Open Collective at
